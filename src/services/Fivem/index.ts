/* eslint-disable promise/param-names, no-new */
...
import {
  masterListFullServerData2ServerView,
  masterListServerData2ServerView,
...
const readBodyToServers = async (
  readerBody: ReadableStream<Uint8Array>,
): Promise<ServerCitizenSchemaType[]> => {
...
export async function getAllMasterListServers(): Promise<
...
export async function getMasterListServer(
  address: string,
): Promise<ServerDynamicSchemaType | null> {
  try {
...
export async function getServerEndpoint(
  cfxHash: string,
): Promise<string | null> {
  'use server'
  if (typeof window !== 'undefined') {
