...
interface PlansContainerProps extends PlanSchemaType {}
  ...
  return (
    <PlanWrapper outlined={plan.tag === 'RECOMMENDED'}>
      <TitleContainer>
        ...
        {
          tagName &&
          <TagContainer primary={plan.tag !== 'FREE'} secondary={plan.tag === 'FREE'}>
            <Text size={'xs'} weight={'bold'} color={'$white'} css={{ lineHeight: '100%' }}>{tagName}</Text>
          </TagContainer>
        }
      </TitleContainer>

      <Heading as={'h3'} weight={'bold'}>{priceFormatted ? priceFormatted : 'Contato'}</Heading>
...

      {
        plan.tag !== 'FREE' ?
          <Button
            size={'lg'}
            onClick={handleOnBuyPlan.bind(null, plan)}
          >
            {priceFormatted ? 'Assinar plano' : 'Entre em contato'}
          </Button>
        :
          <Button size={'lg'} disabled>
            <Text size={'sm'} color={'$neutral200'}>Já assinado</Text>
          </Button>
      }
    </PlanWrapper>
  )
