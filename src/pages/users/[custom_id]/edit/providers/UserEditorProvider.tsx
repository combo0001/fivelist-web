...
export const UserEditorProvider: React.FC<{ children: React.ReactNode, user: UserProfileSchemaType }> = ({
  children,
  user
}) => {
  ...
  const refreshUser = useCallback(
    async (): Promise<void> => {
      const user = await utils.users.getUserProfile.fetch({ customId: userInEdit.customId })
