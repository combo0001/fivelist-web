...
export const ServersListProvider: React.FC<{
  ...
  newServers: any[]
}> = ({ children, newServers, servers: previewServers }) => {
    ...
    async (pageId: string): Promise<boolean> => {
      ...
      setServers((state) => {
        ...
        return state.map((server) => {
          if (!server.preview?.page || server.preview.page.id !== pageId)
            return server
  ...
  useEffect(() => {
    ...
    getAllMasterListServers().then((citizenServers) => {
      const serversList: ServerViewsSchemaType = citizenServers.map(
        (cfxServer) => {
          const previewServer = previewServers.find(
            (previewServer) => previewServer.joinId === cfxServer.joinId,
          )
...
export const useServersList = () =>
  useContext<ServersListProviderProps>(
    ServersListCtx as Context<ServersListProviderProps>,
  )
